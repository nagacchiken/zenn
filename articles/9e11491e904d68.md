...
cdk migrateコマンドを実行していきます。CDKの言語はTypeScriptを選択。CDKのデファクトスタンダードはTypeScriptのようなので、万一のトラブルシューティングも楽なんじゃないかと思い。また、プロジェクト自体はGoで記述しているのですが、毎度ドキュメントを参照するたびに頭の中で言語を変換して読み替えるコストが高いと思いTypeScriptに寄せる判断をしました。
```
